...
def GetLastVulnValue():
    try:
        LastVulnValue.append(sqlite3.connect(DatabasePath).cursor().execute("SELECT VULNID FROM Vuln ORDER BY VULNID DESC LIMIT 1").fetchone()[0] + 1)
    except Exception as Error:
        Monitor("Error", str(Error), "")

def CheckDataIsExist(ID, DatabaseName, Value):
    try:
        if sqlite3.connect(DatabasePath).cursor().execute("SELECT "+str(ID)+" FROM "+DatabaseName+" WHERE "+str(ID)+" = ?", (Value,)).fetchone() is None:
            return False
        else:
            return True
